...
// --- CEREBRO CAJA: CONVERSIÓN VISUAL ---
...
function obtenerVistaPedido(pedido_actual) {
    ...
    while (expE.length > 0 && expS.length > 0) {
        ...
        let recargo = 0; let modName = 'local'; let nota = '';

        if (E.modalidad === 'delivery' || S.modalidad === 'delivery') {
            recargo = 3; modName = 'delivery';
            if (E.modalidad !== S.modalidad) nota = ` (Mxt)`;
        } else if (E.modalidad === 'llevar' && S.modalidad === 'llevar') {
            recargo = 2; modName = 'llevar';
        } else if (S.modalidad === 'llevar') {
            recargo = 2; modName = 'mixto'; nota = ` (S/Llevar)`;
        } else if (E.modalidad === 'llevar') {
            recargo = 1; modName = 'mixto'; nota = ` (E/Llevar)`;
        }

        let nombreCombo = `Menú Completo${nota}`;
        let idx = vista.findIndex(v => v.nombre === nombreCombo && (v.modalidad||'local') === modName);
        if (idx > -1) { vista[idx].cantidad++; vista[idx].subtotal += (15 + recargo); } 
        else { vista.push({ nombre: nombreCombo, cantidad: 1, precio: 15, modalidad: modName, categoria: 'menu', subtotal: 15 + recargo }); }
    }

    [...expE, ...expS, ...otros].forEach(item => {
        let recargo = calcularRecargoTaper(item.modalidad, item.categoria, item.nombre);
        let idx = vista.findIndex(v => v.nombre === item.nombre && (v.modalidad||'local') === (item.modalidad||'local'));
...
function actualizarComandera() {
    ...
    if (mesa.estado === "libre" || !mesa.pedido_actual || mesa.pedido_actual.length === 0) {
        ...
        totalCuenta.innerText = "S/ 0.00"; btnCobrar.disabled = true;
    } else {
        ...
        totalCuenta.innerText = `S/ ${total.toFixed(2)}`; btnCobrar.disabled = false; 
    }
}

// BOTONES DE EDICIÓN (CAJA ADAPTADA AL COMBO VIRTUAL)
window.modificarVistaCantidad = async (nombre, modalidad, cambio) => {
    ...
    if (cambio === -1) {
        ...
        if (nombre.startsWith('Menú Completo')) {
            ...
            else pedido.push({nombre: nombre, precio: 0, cantidad: 1, modalidad: modalidad, subtotal: 0});
        }
    }
...
window.eliminarVistaItem = async (nombre, modalidad) => {
    ...
    if (nombre.startsWith('Menú Completo')) {
        let cantE = pedido.filter(i => i.categoria === 'entrada').reduce((a,b)=>a+b.cantidad,0);
        let cantS = pedido.filter(i => i.categoria === 'segundo').reduce((a,b)=>a+b.cantidad,0);
...
btnArqueo.addEventListener('click', async () => {
    ...
    try {
        ...
        ventasSnap.forEach((doc) => {
            const data = doc.data();
            if (data.metodos_pago) { totEfectivo += data.metodos_pago.efectivo || 0; totYape += data.metodos_pago.yape || 0; totPlin += data.metodos_pago.plin || 0; totTarjeta += data.metodos_pago.tarjeta || 0; }
        });
        ...
        ventasSnap.forEach((doc) => {
            const data = doc.data();
            if (data.items) { data.items.forEach(item => { if (esPlatoParaRanking(item.nombre)) conteoPlatosHoy[item.nombre] = (conteoPlatosHoy[item.nombre] || 0) + item.cantidad; }); }
        });
...
async function cargarDatosDashboard(anioMes) {
    ...
    try {
        ...
        const numDias = finMes.getDate(); const labelsDias = Array.from({length: numDias}, (_, i) => `Día ${i + 1}`);
        const dataIngresos = new Array(numDias).fill(0); const dataGastos = new Array(numDias).fill(0);
        let conteoPlatos = {};
...
window.imprimirNotaVentaHTML = (mesaNum, itemsVista, total, metodos) => {
    ...
    // IMPRIMIMOS LA VISTA AGRUPADA (Ya está calculada y fusionada)
